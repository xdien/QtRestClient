...
RestClient::RestClient(QObject *parent) : RestClient{DataMode::Json, parent} {}
...
RestClass *RestClient::createClass(const QString &path, QObject *parent) {
  ...
  return new RestClass{
      this, path.split(QLatin1Char('/'), QString::SkipEmptyParts), parent};
#else
  return new RestClass{this, path.split(QLatin1Char('/'), Qt::SkipEmptyParts),
                       parent};
#endif
}
...
RestClient::DataMode RestClient::dataMode() const {
  ...
  QReadLocker _{d->threadLock};
#ifndef Q_RESTCLIENT_NO_JSON_SERIALIZER
  return d->serializer && d->serializer->metaObject()->inherits(
                              &CborSerializer::staticMetaObject)
             ? DataMode::Cbor
             : DataMode::Json;
#else
  return d->dataMode;
...
RestClient::requestAttributes() const {
  Q_D(const RestClient);
  QReadLocker _{d->threadLock};
  return d->attribs;
}
...
void RestClient::setSerializer(SerializerBase *serializer) {
  ...
  _.unlock();
  Q_EMIT dataModeChanged(dataMode(), QPrivateSignal{});
}
#endif
...
void RestClient::setRequestAttributes(
...
void RestClient::addGlobalHeader(const QByteArray &name,
                                 const QByteArray &value) {
  Q_D(RestClient);
  QWriteLocker _{d->threadLock};
  d->headers.insert(name, value);
  Q_EMIT globalHeadersChanged(d->headers, RestClient::QPrivateSignal{});
}
...
void RestClient::addGlobalParameter(const QString &name, const QString &value) {
  ...
  Q_EMIT globalParametersChanged(d->query, RestClient::QPrivateSignal{});
}
...
void RestClient::addRequestAttribute(QNetworkRequest::Attribute attribute,
                                     const QVariant &value) {
  Q_D(RestClient);
  QWriteLocker _{d->threadLock};
  d->attribs.insert(attribute, value);
  Q_EMIT requestAttributesChanged(d->attribs, RestClient::QPrivateSignal{});
}
...
/*!
@param name The name to identify the API with. Use to obtain a reference to the
API later on
@param client The RestClient to be registered
@returns `true` if added, `false` if the name is already taken

@attention QtRestClient takes ownership of the `client`, do not delete it after
adding it
...
If you don't delete the client, It will be automatically deleted together with
the QCoreApplication. If you don't want this, simply retake ownership by
yourself:
...
@returns The `RestClient` for the given API name, or `nullptr` if no such API
exists
...
/*!
@param name The name of the root class to be returned
@returns The root `RestClass` for the given API name, or `nullptr` if no such
API exists
...
@returns A newly created `RestClass` for the given API name, or `nullptr` if no
such API exists
