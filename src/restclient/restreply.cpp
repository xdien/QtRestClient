...
RestReply::RestReply(const QFuture<QNetworkReply *> &networkReplyFuture,
                     QObject *parent)
    : RestReply{*new RestReplyPrivate{}, parent} {
  Q_D(RestReply);
  if (networkReplyFuture.isFinished()) {
    ...
    connect(
        d->watcher, &QFutureWatcherBase::finished, this,
        [this]() {
          Q_D(RestReply);
...
RestReply::RestReply(QNetworkReply *networkReply, QThreadPool *asyncPool,
                     QObject *parent)
...
RestReply::RestReply(const QFuture<QNetworkReply *> &networkReplyFuture,
                     QThreadPool *asyncPool, QObject *parent)
...
RestReply *RestReply::onError(
    QObject *scope,
    std::function<void(QString, int, RestReply::Error)> handler) {
  connect(
      this, &RestReply::error, scope,
      [xHandler = std::move(handler)](const QString &errorString, int error,
                                      Error type) {
...
RestReply *RestReply::onAllErrors(
    const std::function<void(QString, int, RestReply::Error)> &handler) {
...
RestReply *RestReply::onAllErrors(
    ...
    const std::function<void(QString, int, RestReply::Error)> &handler) {
  this->onFailed(
      scope, [handler](int code) { handler(QString{}, code, Error::Failure); });
...
RestReplyAwaitable RestReply::awaitable() { return RestReplyAwaitable{this}; }
...
const QByteArray RestReplyPrivate::PropertyBuffer(
    "__QtRestClient_RestReplyPrivate_PropertyBuffer");

QNetworkReply *RestReplyPrivate::compatSend(QNetworkAccessManager *nam,
...
#ifdef QT_RESTCLIENT_USE_ASYNC
void RestReplyPrivate::compatSendAsync(
    QFutureInterface<QNetworkReply *> futureIf, QNetworkAccessManager *nam,
  ...
  if (QThread::currentThread() == nam->thread()) {
    ...
    auto helper = new AsyncHelper{
        [xfif = std::move(futureIf), nam, request, verb, body]() {
          auto fif = xfif;
          auto rep = compatSend(nam, request, verb, body);
...
RestReplyPrivate::RestReplyPrivate() { setAutoDelete(false); }

void RestReplyPrivate::connectReply() {
  Q_Q(RestReply);
  connect(networkReply, &QNetworkReply::finished, this,
          &RestReplyPrivate::_q_replyFinished);

  // forward some signals
#if QT_VERSION < QT_VERSION_CHECK(5, 15, 0)
  QObject::connect(
      networkReply,
      QOverload<QNetworkReply::NetworkError>::of(&QNetworkReply::error), q,
      &RestReply::networkError);
#else
  QObject::connect(networkReply, &QNetworkReply::errorOccurred, q,
                   &RestReply::networkError);
#endif
#ifndef QT_NO_SSL
  connect(networkReply, &QNetworkReply::sslErrors, this,
          &RestReplyPrivate::_q_handleSslErrors);
#endif
  QObject::connect(networkReply, &QNetworkReply::downloadProgress, q,
                   &RestReply::downloadProgress);
  QObject::connect(networkReply, &QNetworkReply::uploadProgress, q,
...
void RestReplyPrivate::_q_retryReply() {
  ...
  auto verb =
      request
          .attribute(QNetworkRequest::CustomVerbAttribute, RestClass::GetVerb)
          .toByteArray();
  auto body = networkReply->property(PropertyBuffer).toByteArray();

  qCDebug(logReply) << "Retrying request with HTTP-Verb:" << verb.constData();
...
void RestReplyPrivate::run() {
  ...
  auto contentType = networkReply->header(QNetworkRequest::ContentTypeHeader)
                         .toByteArray()
                         .trimmed();
  const auto contentLength =
      networkReply->header(QNetworkRequest::ContentLengthHeader).toInt();

  qCDebug(logReply) << "Received reply with status" << status
                    << "and content of type" << contentType << "with length"
                    << contentLength;
    ...
    for (auto i = 1; i < cList.size(); ++i) {
      ...
      if (args.size() == 2 && args[0] == "charset") {
        if (args[1].toLower() != "utf-8") {
          parseError =
              std::make_pair(-1, QStringLiteral("Unsupported charset: %1")
  ...
  if (parseError) {
  ...
  } else if (contentLength == 0 && (status == 204 || status >= 300 ||
                                    allowEmptyReplies)) { // 204 = NO_CONTENT
    ...
    if (error.error != QJsonParseError::NoError) {
      ...
      if (error.error == QJsonParseError::IllegalValue) {
        ...
        jDoc = QJsonDocument::fromJson("[" + readData + "]",
                                       &error); // read wrapped as array
        if (error.error == QJsonParseError::NoError) {
          parseError.reset();
    ...
    parseError =
        std::make_pair(-1, QStringLiteral("Unsupported content type: %1")
                               .arg(QString::fromUtf8(contentType)));

  // check "http errors", because they can have data, but only if json is valid
  if (!parseError && status >= 300 &&
      !std::holds_alternative<std::nullopt_t>(
          data)) // first: status code error + valid data
    Q_EMIT q->failed(status, data, RestReply::QPrivateSignal{});
  else if (networkReply->error() !=
           QNetworkReply::NoError) // next: check normal network errors
    Q_EMIT q->error(networkReply->errorString(), networkReply->error(),
                    Error::Network, RestReply::QPrivateSignal{});
  else if (parseError) { // next: parsing errors
    Q_EMIT q->error(parseError->second, parseError->first, Error::Parser,
                    RestReply::QPrivateSignal{});
  } else if (status >= 300 && std::holds_alternative<std::nullopt_t>(data))
    Q_EMIT q->failed(
        status, data,
        RestReply::QPrivateSignal{}); // only pass as failed without data if
                                      // any other error does not match
  ...
  if (retryDelay == 0ms) {
    ...
    qCDebug(logReply) << "Retrying request in" << retryDelay.count()
                      << "milliseconds";
    ...
    connect(sTimer, &QTimer::timeout, this, &RestReplyPrivate::_q_retryReply);
    QObject::connect(sTimer, &QTimer::timeout, sTimer, &QTimer::deleteLater);
    retryDelay = -1ms;
...
AsyncHelper::AsyncHelper(std::function<void()> &&fn) : _fn{std::move(fn)} {}
