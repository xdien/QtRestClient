...
void PagingModel::initialize(const QUrl &initialUrl,
                             IPagingModelFetcher *fetcher, int typeId) {
  Q_D(PagingModel);
  beginResetModel();
  d->typeId = typeId;
...
void PagingModel::initialize(const QUrl &initialUrl, RestClass *restClass,
                             int typeId) {
  initialize(initialUrl, new RestClassFetcher{restClass}, typeId);
}

void PagingModel::initialize(RestReply *reply, IPagingModelFetcher *fetcher,
                             int typeId) {
  Q_D(PagingModel);
  beginResetModel();
  d->typeId = typeId;
  ...
  d->generateRoleNames();
  endResetModel();
  reply->onSucceeded(this,
                     [this](int code, const RestReply::DataType &replyData) {
                       Q_D(PagingModel);
                       d->processReply(code, replyData);
                     });
  reply->onAllErrors(this, [this](const QString &message, int code,
...
void PagingModel::initialize(RestReply *reply, RestClass *restClass,
                             int typeId) {
  initialize(reply, new RestClassFetcher{restClass}, typeId);
}

void PagingModel::initialize(IPaging *paging, IPagingModelFetcher *fetcher,
                             int typeId) {
  Q_D(PagingModel);
  beginResetModel();
  d->typeId = typeId;
...
void PagingModel::initialize(IPaging *paging, RestClass *restClass,
                             int typeId) {
  ...
  if (d->typeId == QMetaType::UnknownType) {
#endif
    // handle special case: json value (only mode available without the
    // serializer)
    ...
    if (pName.isEmpty() && role == Qt::DisplayRole) {
      ...
      if (userProp.isValid())
        ...
        return QStringLiteral("%1 <%L2>")
            .arg(QString::fromUtf8(metaObject->className()), index.row());
    }
...
void PagingModel::addRole(int column, int role, const char *propertyName) {
  Q_D(PagingModel);
  Q_ASSERT_X(column < d->columns.size(), Q_FUNC_INFO,
             "Cannot add role to non existant column!");
  d->roleMapping[column].insert(role, propertyName);
  if (!d->data.isEmpty())
    Q_EMIT dataChanged(this->index(0, column),
                       this->index(d->data.size() - 1, column), {role});
}
...
void PagingModelPrivate::clearData() {
  ...
  if (tFlags.testFlag(QMetaType::PointerToQObject) ||
    ...
    for (const auto &value : std::as_const(data)) {
...
void PagingModelPrivate::requestNext() {
  ...
  if (reply) {
    ...
    reply->onAllErrors(q, [this](const QString &message, int code,
                                 RestReply::Error errorType) {
...
void PagingModelPrivate::processReply(int, const RestReply::DataType &data) {
  ...
  try {
    paging = std::visit(
        __private::overload{
            [](std::nullopt_t) -> IPaging * { return nullptr; },
            [&](const auto &vData) {
              return fetcher->client()->pagingFactory()->createPaging(
                  fetcher->client()->serializer(), vData);
            }},
        data);
  } catch (DeserializationException &e) {
    qCCritical(logPagingModel)
        << "Failed to parse received paging object with error:" << e.what();
    Q_EMIT q->fetchError(PagingModel::QPrivateSignal{});
    return;
  }
#else
  auto paging = std::visit(
      __private::overload{
          [](std::nullopt_t) -> IPaging * { return nullptr; },
...
void PagingModelPrivate::processPaging(IPaging *paging) {
  ...
  if (paging->offset() < data.size()) {
    qCWarning(logPagingModel)
        << "Pagings out of sync - dropping duplicate data";
    data = data.mid(0, static_cast<int>(paging->offset()));
  } else if (paging->offset() > data.size()) {
    if (paging->hasPrevious()) {
      qCWarning(logPagingModel)
          << "Pagings out of sync - trying for previous data";
      nextUrl = paging->previous();
      requestNext();
      return;
    } else {
      qCWarning(logPagingModel)
          << "Pagings out of sync - skipping" << paging->offset() - data.size()
  ...
  std::visit(
      [&](const auto &items) {
        q->beginInsertRows({}, data.size(),
                           static_cast<int>(data.size() + items.size() - 1));
#ifndef Q_RESTCLIENT_NO_JSON_SERIALIZER
        ...
        for (const auto &item : items) {
          try {
            ...
            qCCritical(logPagingModel)
                << "Failed to deserialize paging element with error:"
                << e.what();
...
void PagingModelPrivate::processError(const QString &message, int code,
                                      RestReply::Error errorType) {
  Q_Q(PagingModel);
  qCCritical(logPagingModel)
      << "Network request failed with error of type" << errorType << "and code"
      << code << "- error message is:" << qUtf8Printable(message);
  Q_EMIT q->fetchError(PagingModel::QPrivateSignal{});
}
